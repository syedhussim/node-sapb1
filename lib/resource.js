...
class Resource{
     ...
     * @param {function(object,number):void} error - The callback that handles connection and SAP errors.
     */
    put(id, data, success = () => {}, error = () => {}){
        let url = this._baseUrl + this._resourceName + '(' + this._escape(id) + ')';
        this._execute('PUT', url, data, 204, success, error);
    }
     ...
     * @param {function(object,number):void} error - The callback that handles connection and SAP errors.
     */
    update(id, data, success = () => {}, error = () => {}, headers = {}){
        let url = this._baseUrl + this._resourceName + '(' + this._escape(id) + ')';
        this._execute('PATCH', url, data, 204, success, error, headers);
    }
    ...
    _execute(httpMethod, url, data, statusCode, success = () => {}, error = () => {}, headers = {}){
        request.execute(httpMethod, url, data, response => {
            ...
            error(err, 1);
        }, this._b1Session, this._config.ca, headers);
    }
